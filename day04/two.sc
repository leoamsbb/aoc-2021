...
def getBingoBoard(boards: List[Board], lastDrawn: Int, input: Seq[Int]): (Int, Board) = {
  val newBoards = boards.map(_.draw(input.head))
  val nextBoards = newBoards.filterNot(_.isBingo)

  if (nextBoards.isEmpty)
    (input.head, newBoards.head)
  else
    getBingoBoard(nextBoards, input.head, input.tail)
}
